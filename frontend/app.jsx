        ...
        function App() {
            ...
            const checkSession = async () => {
                try {
                    ...
                    setAuthenticated(Boolean(data && data.valid));
                } catch {
                    setAuthenticated(false);
                }
            };
            ...
            if (!authenticated) {
                return (
                    <div style={{ minHeight: '100vh', background: '#f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
                        <div style={{ background: 'white', borderRadius: 12, padding: '40px 48px', width: 360, boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
                            <div style={{ textAlign: 'center', marginBottom: 32 }}>
                                <div style={{ fontSize: 32, marginBottom: 8 }}>🏛️</div>
                                <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: '#111827' }}>DC Policy Tracker</h1>
                                <p style={{ margin: '6px 0 0', fontSize: 13, color: '#6b7280' }}>Enter your access password to continue</p>
                            </div>
                            <form onSubmit={handleLogin}>
                                <input
                                    ...
                                    autoFocus
                                    style={{ width: '100%', padding: '10px 12px', fontSize: 14, border: '1px solid #d1d5db', borderRadius: 6, outline: 'none', boxSizing: 'border-box', marginBottom: 12 }}
                                />
                                {passwordError && <p style={{ margin: '0 0 12px', fontSize: 13, color: '#dc2626' }}>{passwordError}</p>}
                                <button
                                    type="submit"
                                    disabled={passwordLoading || !passwordInput}
                                    style={{ width: '100%', padding: '10px', background: passwordLoading || !passwordInput ? '#a5b4fc' : '#4f46e5', color: 'white', border: 'none', borderRadius: 6, fontSize: 14, fontWeight: 600, cursor: passwordLoading || !passwordInput ? 'default' : 'pointer' }}
                                >
                                    {passwordLoading ? 'Checking…' : 'Sign In'}
                                </button>
                            </form>
                        </div>
                    </div>
        ...
        function DCPolicyTracker({ onLogout }) {
            ...
            const [trackedAgencies, setTrackedAgencies] = useState([]);
                ...
                try {
                    ...
                    if (trackedData) {
                        ...
                        const displayItems = trackedData.map(item => ({
                            ...
                            committees: window.DCPCAFrontend.normalizeCommittees(item.committees), date: item.date,
                            ...
                            coIntroducers: item.co_introducers || null,
                            ...
                            trackedAt: item.tracked_at || null
                        }));
            ...
            const loadCouncilPeriods = async () => {
                try {
                    setLoading(true);
                    const data = await window.DCPCAFrontend.proxyFetch('/CouncilPeriods');
            ...
            const fetchLegislationDetails = async (legislationNumber) => {
                return await window.DCPCAFrontend.proxyFetch(`/LegislationDetails/${legislationNumber}`, 'GET');
            };
            ...
            const checkHearingsForTrackedItems = async () => {
                ...
                for (let i = 0; i < trackedIds.length; i += BATCH_SIZE) {
                    ...
                    await Promise.all(batch.map(async (id) => {
                        try {
                            ...
                            const hearing = window.DCPCAFrontend.extractNextHearing(details);
                            ...
                            const latestActivity = window.DCPCAFrontend.extractLatestActivityDate(details);
                            ...
                            const activityTimeline = window.DCPCAFrontend.extractActivityTimeline(details);
                            const introducedBy = window.DCPCAFrontend.parseMembers(details.introducers) || currentItem?.introducedBy || null;
                            const coIntroducers = window.DCPCAFrontend.parseMembers(details.coIntroducers) || null;

                            newHearingData[id] = { ...hearing, checkedAt: now };
                            const response = await window.DCPCAFrontend.appDataRequest({
                                    action: 'trackedItem.hearing.persist', itemId: id,
                                    hearingCheckedAt: now,
                                    nextHearingDate: hearing.date ? hearing.date.toISOString() : null,
                                    hearingType: hearing.type || null,
                                    hearingLocation: hearing.location || null,
                                    additionalInformation: details.additionalInformation || null,
                                    committeeReReferral: reReferrals.length > 0 ? reReferrals : null,
                                    latestActivityDate: latestActivityDate,
                                    latestActivityLabel: latestActivityLabel,
                                    activityCount: activityCount,
                                    activityTimeline: activityTimeline.length > 0 ? activityTimeline : null,
                                    ...(introducedBy ? { introducedBy } : {}),
                                    coIntroducers: coIntroducers,
                                    ...(details.status ? { status: details.status } : {})
                            });
                            if (!response.ok) throw new Error('Failed to persist hearing');
                            ...
                            setItems(prev => prev.map(i => i.id === id ? {
                                ...
                                introducedBy: introducedBy || i.introducedBy,
                                coIntroducers: coIntroducers,
                ...
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({ action: 'trackedItem.hearings.audit', checked: trackedIds.length, withUpcoming: withHearings });
                    if (!response.ok) console.error('Error logging activity:', response.status);
            ...
            const checkHearingForItem = async (itemId) => {
                try {
                    ...
                    const response = await window.DCPCAFrontend.appDataRequest({
                            action: 'trackedItem.hearing.persist', itemId,
                            hearingCheckedAt: now,
                            nextHearingDate: hearing.date ? hearing.date.toISOString() : null,
                            hearingType: hearing.type || null,
                            hearingLocation: hearing.location || null,
                ...
                try {
                    ...
                    if ((searchMode === 'all' || searchMode === 'keywords') && trackedKeywords.length > 0) {
                        for (const keyword of trackedKeywords) {
                            try {
                                const result = await window.DCPCAFrontend.proxyFetch('/SearchLegislation', 'POST', {
                        ...
                        for (const committee of trackedCommittees) {
                            try {
                                ...
                                allResults.push((data || []).map(row => ({
                                    ...
                                    status: row.status,
                                    introducers: row.introduced_by,
                                    coIntroducers: row.co_introducers,
                    ...
                    if ((searchMode === 'all' || searchMode === 'sponsors') && trackedSponsors.length > 0) {
                        ...
                        for (const sponsor of trackedSponsors) {
                            try {
                                ...
                                console.error(`Error searching sponsor "${sponsor}" in cache:`, err);
                                allResults.push([]);
                            }
                        }
                    }
                    ...
                    const transformedItems = allLegislation.map(leg => {
                        return window.DCPCAFrontend.transformLimsSearchItem(leg, isNewItem);
                    });
                    ...
                    setCurrentSearchCategory('search');
                    setHasMoreResults(false); // keyword/committee/sponsor searches load all results at once
                    setLoading(false);
                } catch (err) {
            ...
            const quickSearchByCategory = async (categoryName, offset = 0, append = false) => {
                ...
                try {
                    const results = await window.DCPCAFrontend.proxyFetch('/SearchLegislation', 'POST', {
            ...
            const toggleSelection = async (itemId) => {
                ...
                try {
                    if (isCurrentlySelected) {
                        const response = await window.DCPCAFrontend.appDataRequest({ action: 'trackedItem.untrack', itemId, itemTitle: item.title });
                        ...
                        const response = await window.DCPCAFrontend.appDataRequest({
                                ...
                                committees: item.committees ?? null,
                                date: item.date ?? null,
                                description: item.description ?? null,
                                link: item.link ?? null,
                                source: item.source,
                                agency: item.agency ?? null,
                                isManualEntry: item.source === 'Municipal Register',
                                isNew: item.isNew ?? null,
                                introducedBy: item.introducedBy ?? null
                            });
                        const data = await response.json();
                        if (!response.ok) throw new Error(data?.error || 'Failed to update tracking');
                        const newSelected = new Set(selectedItems);
            ...
            const updatePriority = async (itemId, newPriority) => {
                ...
                try {
                    ...
                    const data = await response.json();
                    if (!response.ok) throw new Error(data?.error || 'Failed to update priority');
            ...
            const updateActionStatus = async (itemId, newStatus) => {
                ...
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({
                            action: 'trackedItem.actionStatus.update', itemId,
                            itemTitle: item.title, oldStatus: item.actionStatus, newStatus
                        });
                    const data = await response.json();
                    if (response.ok || data?.trackedItemUpdated === true) {
                        setItems(items.map(i => i.id === itemId ? { ...i, actionStatus: newStatus } : i));
                    }
                    if (!response.ok) throw new Error(data?.error || 'Failed to update action status');
            ...
            const updateItemActivity = async (itemId, newStatus, activitySummary) => {
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({
                            action: 'trackedItem.activity.detected', itemId, newStatus,
                            activitySummary, lastCheckedAt: new Date().toISOString()
                    });
                    if (!response.ok) throw new Error('Failed to update item activity');
                } catch (err) { console.error('Error updating item activity:', err); }
            };

            const markActivityAsSeen = async (itemId) => {
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({ action: 'trackedItem.activity.markSeen', itemId });
                    const data = await response.json();
                    if (!response.ok) throw new Error(data?.error || 'Failed to mark activity as seen');
            ...
            const saveNote = async () => {
                ...
                const activityAction = itemNotes[editingNote] ? 'note_updated' : 'note_added';
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({
                            action: 'note.save',
                            itemId: editingNote,
            ...
            const saveSummary = async () => {
                ...
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({ action: 'trackedItem.summary.save', itemId: editingSummary, summary: summaryText || null });
                    const data = await response.json();
            ...
            const deleteManualSummary = async (itemId) => {
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({ action: 'trackedItem.summary.delete', itemId });
                    const data = await response.json();
                    if (!response.ok) throw new Error(data?.error || 'Failed to delete summary');
            ...
            const addKeyword = async () => {
                ...
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({ action: 'keyword.add', keywords: incoming });
                    const data = await response.json();
                    if (!response.ok) throw new Error(data?.error || 'Failed to add keywords');
            ...
            const removeKeyword = async (keyword) => {
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({ action: 'keyword.remove', keyword });
                    const data = await response.json();
                    if (!response.ok) throw new Error(data?.error || 'Failed to remove keyword');
            ...
            const addCommittee = async () => {
                ...
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({ action: 'committee.add', committeeName: newCommittee });
                    const data = await response.json();
                    if (!response.ok) throw new Error(data?.error || 'Failed to add committee');
            ...
            const removeCommittee = async (committee) => {
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({ action: 'committee.remove', committeeName: committee });
                    const data = await response.json();
                    if (!response.ok) throw new Error(data?.error || 'Failed to remove committee');
            ...
            const addSponsor = async () => {
                ...
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({ action: 'sponsor.add', sponsorName: newSponsor });
                    const data = await response.json();
            ...
            const removeSponsor = async (sponsor) => {
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({ action: 'sponsor.remove', sponsorName: sponsor });
                    const data = await response.json();
                    if (!response.ok) throw new Error(data?.error || 'Failed to remove sponsor');
            ...
            const addAgency = async (name) => {
                ...
                if (!trimmed || trackedAgencies.includes(trimmed)) return;
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({ action: 'agency.add', agencyName: trimmed });
                    const data = await response.json();
                    if (!response.ok) throw new Error(data?.error || 'Failed to add agency');
                    setTrackedAgencies([...trackedAgencies, trimmed].sort());
                } catch (err) { setError('Failed to add agency: ' + err.message); }
            };

            const removeAgency = async (name) => {
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({ action: 'agency.remove', agencyName: name });
                    const data = await response.json();
                    if (!response.ok) throw new Error(data?.error || 'Failed to remove agency');
                    setTrackedAgencies(trackedAgencies.filter(a => a !== name));
                } catch (err) { setError('Failed to remove agency: ' + err.message); }
            };
            ...
            const addManualEntry = async () => {
                ...
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({
                            action: 'trackedItem.manual.create',
                            itemId: newItem.id,
                            title: newItem.title,
                            ...
                            actionStatus: newItem.actionStatus,
                            noticeId: newItem.noticeId,
                            registerIssue: newItem.registerIssue,
                            registerNotes: newItem.registerNotes,
                            deadline: newItem.deadline,
                            isNew: newItem.isNew,
                            latestActivityDate: newItem.latestActivityDate
                        });
                    const data = await response.json();
                    if (!response.ok) throw new Error(data?.error || 'Failed to add manual entry');
            ...
            const updateManualEntry = async () => {
                ...
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({
                            action: 'trackedItem.manual.update',
                            itemId: editingManualEntry,
                    ...
                    const data = await response.json();
                    if (!response.ok) throw new Error(data?.error || 'Failed to update manual entry');
            ...
            const deleteManualEntry = async (itemId) => {
                ...
                try {
                    const response = await window.DCPCAFrontend.appDataRequest({ action: 'trackedItem.manual.delete', itemId });
                    const data = await response.json();
                    if (!response.ok) throw new Error(data?.error || 'Failed to delete manual entry');
            ...
            const updateTeamMember = async () => {
                ...
                try {
                    ...
                    const response = await window.DCPCAFrontend.appDataRequest({
                            ...
                            teamMemberId: String(editingTeamMember),
                            name: teamMemberForm.name,
                            email: teamMemberForm.email || null
                        });
                    const data = await response.json();
            ...
            const exportToCSV = () => {
                const headers = [
                    'Title', 'Bill Number', 'Category', 'Status', 'Committees',
                    'Introduced By', 'Co-Introduced By', 'Date Introduced', 'Latest Activity', 'Latest Activity Date',
                    'Next Hearing', 'Hearing Type', 'Hearing Location',
                    'Action Status', 'Priority', 'Assigned To',
                    'Re-Referrals', 'Activity Count',
                    'Bill Summary', 'Notes',
                    'Activity Timeline',
                    'Link', 'Tracked'
                ];
                const rows = filteredItems.map(item => {
                    const h = hearingData[item.id];
                    const reReferrals = (item.committeeReReferral || [])
                        .map(r => `${Array.isArray(r.committeeName) ? r.committeeName.join(', ') : (r.committeeName || '')}${r.reReferralDate ? ' (' + new Date(r.reReferralDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) + ')' : ''}`)
                        .join('; ');
                    const timeline = (item.activityTimeline || [])
                        .map(e => `${e.dateStr}: ${e.label}${e.extra ? ' (' + e.extra + ')' : ''}`)
                        .join(' | ');
                    const summary = item.manualSummary || item.additionalInformation || '';
                    return [
                        item.title,
                        item.billNumber || '',
                        item.category,
                        ...
                        item.latestActivityDate ? new Date(item.latestActivityDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '',
                        h?.dateStr || '',
                        h?.type || '',
                        h?.location || '',
                        item.actionStatus || '',
            ...
            const filteredItems = items.filter(item => {
                ...
                const matchesAgency = filters.agency === 'all' || item.agency === filters.agency;
                ...
                return matchesKeyword && matchesCommittee && matchesAgency && matchesCategory && matchesAssignedTo &&
                ...
                if (sortBy === 'tracked_desc') return new Date(b.trackedAt || 0) - new Date(a.trackedAt || 0);
                if (sortBy === 'tracked_asc') return new Date(a.trackedAt || 0) - new Date(b.trackedAt || 0);
                if (sortBy === 'introduced_asc') return new Date(a.date || 0) - new Date(b.date || 0);
                if (sortBy === 'status') return (a.status || '').localeCompare(b.status || '');
                if (sortBy === 'bill_number') return (a.billNumber || '').localeCompare(b.billNumber || '', undefined, { numeric: true });
                return 0; // default: preserve fetch order
            });
            ...
            const agencies = ['all', ...new Set(items.filter(i => i.agency).map(i => i.agency).filter(Boolean)).values()].sort((a, b) => a === 'all' ? -1 : a.localeCompare(b));
                        ...
                        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    <button onClick={onLogout} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Log Out</button>
                                ...
                                <div className="bg-white rounded-lg shadow-lg p-6">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <select value={filters.agency} onChange={e => setFilters({...filters, agency: e.target.value})} className="w-full px-3 py-2 border rounded-lg">
                                ...
                                <div className="bg-white rounded-lg shadow-lg p-6">
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="px-3 py-1 text-xs rounded border border-gray-300 bg-white text-gray-600 hover:bg-gray-50">
                                                <option value="default">Sort: Default</option>
                                                <option value="introduced_desc">Sort: Intro Date (Newest)</option>
                                                <option value="introduced_asc">Sort: Intro Date (Oldest)</option>
                                                <option value="activity">Sort: Latest Activity</option>
                                                <option value="tracked_desc">Sort: Recently Tracked</option>
                                                <option value="tracked_asc">Sort: First Tracked</option>
                                                <option value="status">Sort: Status A–Z</option>
                                                <option value="bill_number">Sort: Bill Number</option>
                                            </select>
                                        </div>
                                    </div>
                                                    ...
                                                    <div className="flex items-start gap-3">
                                                        ...
                                                        <div className="flex-1">
                                                            ...
                                                            <div className="text-xs text-gray-500 mb-2">
                                                                ...
                                                                {item.coIntroducers && <span className="ml-2 text-gray-400">• Co-introduced by: {item.coIntroducers}</span>}
                                                                {latestActivityStr && (
                                                            ...
                                                            {item.isManualEntry && item.agency && (
                                                                <div className="mb-2">
                                                                    <div className="text-xs font-semibold mb-1">Agency:</div>
                                                                    <span className="px-2 py-0.5 bg-purple-100 text-purple-700 text-xs rounded">{item.agency}</span>
                                                                </div>
                                                            )}
                                                            {!item.isManualEntry && item.committees && item.committees.length > 0 && (
                                                                <div className="mb-2">
                    ...
                    {showAddSponsor && (
                        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                            <div className="bg-white rounded-lg p-6 max-w-md w-full">
                                ...
                                <p className="text-sm text-gray-600 mb-1">Enter the councilmember's last name or full name</p>
                                <p className="text-xs text-amber-600 mb-3">⚠️ Sponsor search requires the bill cache to be built. If this is your first time using sponsor search, manually trigger <code className="bg-gray-100 px-1 rounded">/api/build-bill-cache</code> via your Vercel dashboard, or wait for the nightly cron (midnight ET).</p>
                            ...
                            <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-screen overflow-y-auto">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-2">
                                            <select
                                                value={trackedAgencies.includes(manualEntry.agency) ? manualEntry.agency : (manualEntry.agency ? '__custom__' : '')}
                                                onChange={e => {
                                                    if (e.target.value === '__add__') {
                                                        setShowAddAgency(true);
                                                    } else if (e.target.value !== '__custom__') {
                                                        setManualEntry({...manualEntry, agency: e.target.value});
                                                    }
                                                }}
                                                className="flex-1 px-3 py-2 border rounded-lg"
                                            >
                                                <option value="">Select an agency...</option>
                                                {trackedAgencies.map(a => <option key={a} value={a}>{a}</option>)}
                                                {manualEntry.agency && !trackedAgencies.includes(manualEntry.agency) && (
                                                    <option value="__custom__">{manualEntry.agency}</option>
                                                )}
                                                <option value="__add__">+ Add new agency to list...</option>
                                            </select>
                                        </div>
                                        {showAddAgency && (
                                            <div className="mt-2 flex gap-2">
                                                <input
                                                    ...
                                                    onKeyDown={e => {
                                                        if (e.key === 'Enter') {
                                                            addAgency(newAgencyInput).then(() => {
                                                                setManualEntry({...manualEntry, agency: newAgencyInput.trim()});
                                                                setNewAgencyInput('');
                                                                setShowAddAgency(false);
                                                            });
                                                        }
                                                        if (e.key === 'Escape') { setShowAddAgency(false); setNewAgencyInput(''); }
                                                    }}
                                                />
                                                <button
                                                    onClick={() => addAgency(newAgencyInput).then(() => {
                                                        setManualEntry({...manualEntry, agency: newAgencyInput.trim()});
                                                        setNewAgencyInput('');
                                                        setShowAddAgency(false);
                                                    })}
                                                ...
                                                >Save</button>
                                                <button onClick={() => { setShowAddAgency(false); setNewAgencyInput(''); }} className="px-3 py-2 bg-gray-200 rounded-lg text-sm hover:bg-gray-300">Cancel</button>
                                            </div>
                                        )}
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
