...
The browser tests evaluate the existing parser, committee normalization, date, and search-item transformation source from `index.html` inside a small VM. The cron tests evaluate the existing `api/check-hearings.js` source with mocked fetch and mail. Runtime files and API contracts are unchanged. The test clock is 2026-09-21 12:00 UTC; the test worker uses UTC for formatted dates. The cron delay is skipped only in the test harness.
